...
/*
 * Takes in a 3d mesh and outputs a vector of all the nodes to be cut by the toolpath
*/
void ToolPath::create_nodes(C3t3& mesh_3d, std::unordered_map<Tr::Vertex_handle, size_t> &node_id_map)
{
	Surface_mesh mesh;
...
std::vector<size_t> ResidualCalculator::get_elems_in_mesh(const Surface_mesh &mesh)
{
    CGAL::AABB_tree<CGAL::AABB_traits_3<Kernel, CGAL::AABB_face_graph_triangle_primitive<Surface_mesh>>> 
    AABB_tree(faces(mesh).begin(), faces(mesh).end(), mesh);

    AABB_tree.accelerate_distance_queries();
    CGAL::Side_of_triangle_mesh<Surface_mesh, Kernel> inside_test(AABB_tree);

    std::vector<size_t> elems_inside_mesh;
    for (size_t i = 0; i < elem10.size(); i++)
    {
	const Ten& elem = elem10[i];

        auto v0 = id_node_map.at(elem.v[0]);
        auto v1 = id_node_map.at(elem.v[1]);
        auto v2 = id_node_map.at(elem.v[2]);
        auto v3 = id_node_map.at(elem.v[3]);
        ...
        // Tetra centroid (barycenter of 4 corners)
        Point middle = CGAL::barycenter(p0, 1.0, p1, 1.0, p2, 1.0, p3, 1.0);
	auto side = inside_test(middle);

	if (side == CGAL::ON_BOUNDED_SIDE || side || CGAL::ON_BOUNDARY)
	{
	    elems_inside_mesh.push_back(i+1);
	}
    }

    return elems_inside_mesh;
}

std::unordered_map<Tr::Vertex_handle, StressTensor> ResidualCalculator::full_material_stress_estimator()
{
    std::string calculix_input_file {"calculix"};
    std::ofstream inp(calculix_input_file + ".inp");
    ...
    int node_id = 1;
    std::unordered_map<Point, int> node_index;
    for (auto vit = mesh_c3t3.triangulation().finite_vertices_begin(); vit != mesh_c3t3.triangulation().finite_vertices_end(); ++vit)
    {
	Point point = vit->point().point();
	node_index[point] = node_id;
        inp << node_id++ << ", " << point.x() << ", " << point.y() << ", " << point.z() << "\n";
    }

    inp << "*ELEMENT, TYPE=C3D10\n";
    int elem_id = 1;
    for (auto cit = mesh_c3t3.cells_in_complex_begin(); cit != mesh_c3t3.cells_in_complex_end(); ++cit)
    {
        inp << elem_id++ << ", ";
        for (int i = 0; i < 4; ++i) 
	{
            Point point = cit->vertex(i)->point().point();
            inp << node_index[point];
            if (i < 3) inp << ", ";
	}
    ...
    inp << "*SOLID SECTION, ELSET=ALL, MATERIAL=MAT1\n";

    inp << "*BOUNDARY\n1,1,3,0.0\n";

    inp << "*DLOAD\n";
    for (auto it = surface_stress_map.begin(); it != surface_stress_map.end(); ++it)
    {
        const Point& p = it->first;
        const StressTensor& s = it->second;
        // Get node normal (approximate: direction of position vector normalized)
        K::Vector_3 n = CGAL::unit_normal(p, Point(p.x()+1e-6, p.y(), p.z()),
                                             Point(p.x(), p.y()+1e-6, p.z()));
        Eigen::Vector3d nvec(n.x(), n.y(), n.z());
        nvec.normalize();

        Eigen::Matrix3d sigma;
        sigma << s.sxx, s.sxy, s.szx,
                 s.sxy, s.syy, s.syz,
                 s.szx, s.syz, s.szz;

        Eigen::Vector3d t = sigma * nvec;
        inp << node_index[p] << ", PX, " << t(0) << "\n";
        inp << node_index[p] << ", PY, " << t(1) << "\n";
        inp << node_index[p] << ", PZ, " << t(2) << "\n";
    }
    ...
    std::string cmd = ccx_executable + " " + inp_path.substr(0, inp_path.find_last_of('.'));
...
/*
Surface_mesh ResidualCalculator::simulate_cut(const Surface_mesh &tool_path)
{
